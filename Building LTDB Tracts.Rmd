...
Reading permit data
```{r}
megapermit <- read_csv("permitdatajun17.csv")
```
...
```{r}
colortest_hpoz <- census_hpoz |>
  filter(Shape__Are > "5366817")
...
leaflet(data = census_hpoz) |>
  addPolygons(data = colortest_hpoz,
              fillOpacity = 1,
              fillColor = "#d21217",
              color = "black",
              weight = 1.5
...
leaflet(data = census_hpoz) |>
  ...
  addProviderTiles("CartoDB.Positron") |>
  addPolygons(data = hpoz_shp_cleaned,
              fillOpacity = 0.5,
              color = "green",
              weight = 1.5,
              label = hpoz_shp_cleaned$NAME
              ) |>
